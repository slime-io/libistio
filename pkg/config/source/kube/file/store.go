...
import (
	...
	kube2 "istio.io/libistio/pkg/config/legacy/source/kube"
	"istio.io/libistio/pkg/config/resource"
	"istio.io/libistio/pkg/config/schema/collection"
	"istio.io/libistio/pkg/config/source/inmemory"
	"istio.io/libistio/pkg/config/source/kube/file/util/kubeyaml"
	"istio.io/libistio/pkg/kube"
...
func (s *KubeSource) Get(gvk config.GroupVersionKind) *inmemory.Collection {
	return s.source.Get(gvk)
}
...
func (s *KubeSource) ApplyContent(name, yamlText string) error {
	...
	for _, r := range resources {
		...
		if !found || oldSha != r.sha {
			s.versionCtr++
			if r.resource.Metadata.Version == "" {
				r.resource.Metadata.Version = resource.Version(fmt.Sprintf("v%d", s.versionCtr))
			}
			scope.Debugf("KubeSource.ApplyContent: Set: %v/%v", r.schema.GroupVersionKind(), r.resource.Metadata.FullName)
			s.source.Get(r.schema.GroupVersionKind()).Set(r.resource)
			s.shas[key] = r.sha
		}
...
func (s *KubeSource) parseChunk(r *collection.Schemas, name string, lineNum int, yamlChunk []byte) ([]kubeResource, error) {
	...
	c, err := ToResource(objMeta, schema, &pos, fieldMap)
	if err != nil {
...
func ToResource(object metav1.Object, schema schemaresource.Schema, source resource.Reference, fieldMap map[string]int) (*resource.Instance, error) {
	...
	message := TranslateMessage(u, "", schema)
...
func TranslateMessage(obj *unstructured.Unstructured, domainSuffix string, schema schemaresource.Schema) config.Spec {
