...
var scope = log.RegisterScope("inmemory", "in memory source")
...
func NewCollection(s resource2.Schema) *Collection {
	scope.Debugf("Creating in-memory collection: %s ", s.GroupVersionKind())
...
func (c *Collection) Dispatch(handler event.Handler) {
	if scope.DebugEnabled() {
		scope.Debugf("Collection.Dispatch: (collection: %-50v, handler: %T)", c.schema.GroupVersionKind(), handler)
	}
...
func (c *Collection) dispatchEvent(e event.Event) {
	if scope.DebugEnabled() {
		scope.Debugf(">>> Collection.dispatchEvent: (col: %-50s): %v", c.schema.GroupVersionKind(), e)
	}
	if c.handler != nil {
		c.handler.Handle(e)
	}
