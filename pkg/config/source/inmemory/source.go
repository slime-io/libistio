...
type Source struct {
	...
	collections map[config.GroupVersionKind]*Collection
	name        string
}
...
func New(collections collection.Schemas) *Source {
	...
	all := collections.All()
	scope.Debugf("Creating new in-memory source (collections: %d)", len(all))

	s := &Source{
		collections: make(map[config.GroupVersionKind]*Collection),
		name:        name,
	}

	for _, c := range all {
		s.collections[c.GroupVersionKind()] = NewCollection(c)
	}
...
func (s *Source) Get(collection config.GroupVersionKind) *Collection {
	s.mu.Lock()
	defer s.mu.Unlock()
